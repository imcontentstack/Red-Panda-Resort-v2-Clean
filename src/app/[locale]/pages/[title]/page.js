...
import TrackingEventEmitter from "@/components/TrackingEventEmitter";
...
export default function Page() {
   ...
   if (entry?.taxonomies && entry?.taxonomies?.length > 0) {
     entry.taxonomies.forEach((tax) => {
       ...
       if (typeof window !== "undefined" && window.jstag?.send) {
         window.jstag.send({ taxonomy: tax.term_uid });
       }
     });
   }
 }, [entry]);
 ...
 return (
   <div>
     <TrackingEventEmitter entry={entry} />
     ...
     {entry?._applied_variants?.title !== "cs76fdee0e83c5c333" && <Footer />}
